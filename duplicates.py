...
def move_to_trash(dup_groups, folder, auto_confirm=False):
    ...
    if not to_delete:
        print("Nessun file duplicato da spostare nel cestino.")
        return

    print(f"\nTrovati {len(to_delete)} file duplicati:")
    for f in to_delete:
        print(f"  {f}")

    if not auto_confirm:
        choice = input("\nVuoi eliminare i duplicati? (Y/n) ").strip().lower()
        if choice == 'n':
    ...
    for f in to_delete:
        try:
            ...
            print(f"Spostato nel cestino: {f}")
        except Exception as e:
            print(f"Errore spostando {f} nel cestino: {e}")
...
if __name__ == "__main__":
    ...
    if not dup_groups:
        ...
        print(f"\nTrovati {len(dup_groups)} gruppi di duplicati.")
